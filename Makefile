...
build: download prepare
	...
	docker tag fingerprintsoft/phantomjs fingerprintsoft/phantomjs:2.1.1

push:
	docker push fingerprintsoft/phantomjs
	docker push fingerprintsoft/phantomjs:2.1.1

download:
	rm -rf dist/ && mkdir dist/
	curl -skL -A "Mozilla" https://cnpmjs.org/mirrors/phantomjs/phantomjs-2.1.1-linux-x86_64.tar.bz2 | tar -xj -C dist/
	mv dist/phantomjs-2.1.1-linux-x86_64/bin/phantomjs phantomjs && rm -rf dist/

prepare:
	docker rm -f ubuntu-donor || true
	docker run --name ubuntu-donor ubuntu:14.04 bash -c "apt-get update && apt-get install --no-install-recommends -y --force-yes libfontconfig1 libfreetype6 gsfonts" 
